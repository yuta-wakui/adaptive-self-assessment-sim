...
def compare_params(
    ...
    if mode == "ws1":
        ra_col: str = ws_cfg.get("overall_col", "")
    else:
        ra_col: str = ws_cfg.get("current_overall_col", "")
    if not ra_col:
        raise ValueError(f"overall label column must be specified in config.")
    ...
    item_cfg = model_cfg.get("item_model", {})
    overall_cfg = model_cfg.get("overall_model", {})
    ...
    for item_var in item_param_combos:
        for overall_var in overall_param_combos:
            ...
            cfg_g["model"].setdefault("item_model", {})
            cfg_g["model"].setdefault("overall_model", {})

            cfg_g["model"]["item_model"]["type"] = item_type
            cfg_g["model"]["overall_model"]["type"] = overall_type
            cfg_g["model"]["item_model"]["params"] = _merge_params(item_params_base, item_var)
            cfg_g["model"]["overall_model"]["params"] = _merge_params(overall_params_base, overall_var)
            ...
            print(f"  item: type={item_type}, params={cfg_g['model']['item_model']['params']}")
            print(f"  overall: type={overall_type}, params={cfg_g['model']['overall_model']['params']}")
