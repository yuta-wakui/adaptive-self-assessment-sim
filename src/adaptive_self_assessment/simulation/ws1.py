...
def run_ws1_simulation(
    ...
    ra_col: str = ws1_cfg.get("ra_col", "") # 総合評価列
    ca_cols_ws1: List[str] = ws1_cfg.get("ca_cols", []) # 項目補完対象列

    if not ra_col or not ca_cols_ws1:
    ...
    if save_logs and log_dir:
        ...
        log_filename = f"ws1_{skill_name}_rc{rc_str}_ri{ri_str}_logs.csv"
