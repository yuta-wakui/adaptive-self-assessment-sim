# -*- coding: utf-8 -*-
"""
Integration tests for non-adaptive self-assessment simulations.
This module tests the end-to-end functionality of the non-adaptive self-assessment simulations for both
WS1 and WS2.
...
# File: tests/integration/test_non_adaptive_sim.py
# Author: Yuta Wakui
# Date: 2026-02-14
# Description: Integration tests for non-adaptive self-assessment simulations (WS1 and WS2)
...
def test_run_non_adaptive_ws1_simulation(ws1_path, cfg_path):
    print("=== Non-Adaptive WS1 Simulation Test ===")
    ...
    # run simulation
    sim_results, logs_df = run_non_adaptive_ws1_simulation(
    ...
    # non-adaptive specific assertions
    assert float(sim_results["reduction_rate"]) == 0.0
    assert float(sim_results["avg_complemented_questions"]) == 0.0
    assert sim_results["avg_complement_accuracy"] is None
    assert all(logs_df["num_complemented_questions"] == 0)
    assert all(logs_df["num_answered_questions"] == logs_df["total_questions"])

    print("Non-Adaptive WS1 simulation Results: ", sim_results)
...
def test_run_non_adaptive_ws2_simulation(ws2_path, cfg_path):
    print("=== Non-Adaptive WS2 Simulation Test ===")
    ...
    # run simulation
    sim_results, logs_df = run_non_adaptive_ws2_simulation(
    ...
    print("Non-Adaptive WS2 simulation Results: ", sim_results)
